...
export default function UserMenu({ user, onChangePassword, onLogout, scraperData, currentTheme = 'default', themes }) {
  ...
  // NOVO: Stanje za dinamičke podatke o plati
  ...
  // NOVO: Logika za dobijanje 01/2026 i neto iznosa
  useEffect(() => {
    const fetchLatestPayroll = async () => {
      ...
      try {
        const { data, error } = await supabase
          ...
          .eq('driver_name', (user.username || "").toLowerCase());
        ...
        if (!error && data && data.length > 0) {
          ...
          setLatestPayroll({
            amount: sorted[0].neto,
            date: sorted[0].file_name.replace('.pdf', '').replace('.PDF', '').replace('_', '/')
          });
        }
      } catch (err) {
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            style={{ zIndex: 999999, position: 'absolute', top: '100%', right: 0 }}
          >
            {/* Profilna sekcija - Originalni dizajn */}
            ...
            {/* SEKCIJA SA PLATAMA - Popravljeni podaci */}
            <div className="grid grid-cols-2 gap-2 mb-3">
              {user?.role !== "admin" && (
                <>
                  ...
                  >
                    <div className={`text-[10px] font-bold ${currentTheme === 'night' ? 'text-blue-400' : 'text-blue-600'} uppercase`}>
                  ...
                  <motion.div 
                    ...
                    whileHover={{ scale: 1.02 }}
                  >
                    <div className={`text-[10px] font-bold ${currentTheme === 'night' ? 'text-emerald-400' : 'text-emerald-600'} uppercase`}>
                      Ukupna zarada
                    </div>
            ...
            {/* Dugmad - Originalni dizajn */}
            <div className="space-y-1">
              {user?.role === "admin" ? (
                <Button variant="ghost" size="sm" className="w-full justify-start text-blue-600 rounded-xl" onClick={() => handleMenuItemClick(() => navigate("/admin"))}>
                  <Crown className="w-4 h-4 mr-2" /> Admin Panel
                </Button>
              ) : (
                <Button variant="ghost" size="sm" className="w-full justify-start text-emerald-600 rounded-xl" onClick={() => handleMenuItemClick(() => navigate("/payroll-list"))}>
                  <Wallet className="w-4 h-4 mr-2" /> Platne liste
                </Button>
              )}
              <Button variant="ghost" size="sm" className="w-full justify-start text-amber-600 rounded-xl" onClick={() => handleMenuItemClick(onChangePassword)}>
                <KeyRound className="w-4 h-4 mr-2" /> Promijeni šifru
              </Button>
              <Button variant="ghost" size="sm" className="w-full justify-start text-rose-600 rounded-xl" onClick={onLogout}>

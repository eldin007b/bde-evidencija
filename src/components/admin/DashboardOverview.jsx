...
// Pospremi cijeli niz redova - prima urlaubMarks objekt i listu ruta
const cleanRows = (rows, urlaubMarks = {}, routes = []) => {
  ...
  return rows
    ...
    .filter(
      (r) =>
        r.driverClean &&
        (routes.length === 0 || routes.includes(r.driverClean)) &&
...
const DashboardOverview = ({ data, drivers = [], loading, error, onRefresh }) => {
  ...
  // Dinamički dohvatiti dostupne rute iz liste vozača (samo aktivne rute bez admina)
  const availableRoutes = useMemo(() => {
    if (!Array.isArray(drivers) || drivers.length === 0) {
      // Fallback ako još nisu učitani ili nema vozača
      return ['8610', '8620', '8630', '8640'];
    }
    return drivers
      .filter(d => d.role !== 'admin' && d.aktivan)
      .map(d => String(d.tura))
  ...
  // Inicijalizacija routeStats na osnovu availableRoutes
  useEffect(() => {
    const initialStats = {};
    availableRoutes.forEach(route => {
      initialStats[route] = { today: 0, month: 0, year: 0 };
    });
      ...
      if (!latestError && latestRows && latestRows.length > 0) {
        ...
        for (const dateStr of sortedDates) {
          ...
          const hasValidData = rows.some(row => {
            ...
            if (!driver || !availableRoutes.includes(driver)) return false;
  ...
  }, [urlaubMarksLoaded, availableRoutes]);
  ...
  useEffect(() => {
    const fetchStats = async () => {
      ...
      try {
        ...
        const dayRows = cleanRows(dayRaw, urlaubMarks, availableRoutes);
        ...
        const monthRows = cleanRows(monthRaw, urlaubMarks, availableRoutes);
        ...
        const yearRows = cleanRows(yearRaw, urlaubMarks, availableRoutes);
    ...
    fetchStats();
  }, [selectedDay, selectedMonth, selectedYear, urlaubMarks, availableRoutes]);
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div className="text-right flex-shrink-0">
                ...
                <p className="text-xs text-gray-500">
                  {timeRange === 'today'
                    ? `Cilj za ${activeDrivers.length} ture`

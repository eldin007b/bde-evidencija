...
export default function UrlaubTab({ drivers: driversFromProps }) {
  ...
  // Inicijalizacija selekcije
  useEffect(() => {
    if (driversList.length > 0) {
      if (!selectedDriver) setSelectedDriver(driversList[0].id);
      if (!targetDriver && driversList.length > 1) {
        setTargetDriver(driversList[1].id);
      } else if (!targetDriver && driversList.length === 1) {
          setTargetDriver(driversList[0].id);
      }
    }
  }, [driversList]);
  ...
  const handleAddUrlaub = async () => {
    if (!selectedDate || !selectedDriver || !targetDriver) {
      toast.error('Molimo odaberite datum, vozača i ciljnog vozača.');
      return;
    }

    if (selectedDriver === targetDriver) {
      toast.error('Vozač i ciljni vozač ne mogu biti isti.');
      return;
    }
    ...
    try {
      ...
      if (existing) {
        toast.error('Urlaub za ovaj datum i vozača već postoji.');
        setSaving(false);
        return;
      }

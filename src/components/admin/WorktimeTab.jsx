...
export default function WorktimeTab() {
  ...
  /* LOAD DRIVERS */
  useEffect(() => {
    getAllDriversCloud().then(data => {
      const activeDrivers = (data || []).filter(d => d.aktivan && d.role !== 'admin');
      setDrivers(activeDrivers);
      if (activeDrivers.length > 0 && !driver) {
        setDriver(String(activeDrivers[0].tura));
      }
    }).catch(() => setDrivers([]));
  }, []);
  ...
  const selectedDriver = drivers.find(d => String(d.tura) === String(driver));
  const currentDriverName = selectedDriver ? selectedDriver.ime : driver;
  ...
  const sharePDF = async () => {
    try {
      ...
      console.error(e);
      toast.error("Dijeljenje PDF-a nije dostupno");
    }
  };
      ...
      <div className="bg-[#111827] rounded-xl p-4 mb-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
          <select value={driver} onChange={e => setDriver(e.target.value)} className="p-3 rounded bg-[#1f2937]">
            {drivers.map(d => (
              <option key={d.id} value={d.tura}>{d.ime} ({d.tura})</option>
            ))}
          </select>

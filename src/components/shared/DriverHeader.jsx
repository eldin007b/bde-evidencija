...
export default function DriverHeader({ driver, onLogout, onNameClick }) {
  ...
  const isAdmin = driver.role === 'admin' || driver.tura === '8610';

  const handleNameClick = () => {
    if (isAdmin && onNameClick) {
      onNameClick();
    }
  };
  ...
  return (
    <header className="driver-header">
      <div className="driver-header-content">
        <span 
          className={`driver-header-name ${isAdmin ? 'clickable' : ''}`} 
          data-tooltip={isAdmin ? "Kliknite za admin pristup" : "Vaš profil i tura"}

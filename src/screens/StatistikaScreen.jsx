...
import { useDriversQuery } from '../hooks/queries';
...
const StatistikaScreen = () => {
  ...
  // Dinamički dohvati vozače
  const { data: drivers = [], isLoading: loading } = useDriversQuery();
  ...
  const loadDriverStats = useCallback(async (driver) => {
    ...
    try {
      ...
      const targetStops = parseInt(driver?.target_per_day ?? 50, 10) || 0;
  ...
  }, [memoizedCurrentMonth, memoizedCurrentYear, urlaubMarks]);
                ...
                <div className="flex items-center gap-4">
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`text-sm ${isNightTheme ? 'text-gray-400' : 'text-gray-600'}`}>
                        Cilj: {parseInt(selectedDriver?.target_per_day ?? 0, 10) || 0} stopova/dan
                      </span>
                    </div>
                  </motion.div>
